...
class Workflow:
    ...
    def writeDAX(self, filename):
        ...
        for j in self.jobs:
            f.write('\t<job id="%s" namespace="%s" name="%s" runtime="%s" MI= "%s" cores="%d">\n' % (j.id, j.namespace, j.name, j.runtime, j.MI,j.cores))
            for i in j.inputs:
...
if __name__ == '__main__':
    ...
    j = Job(id="foo1", namespace="foo", name="bar", runtime=102.4, MI=10000,inputs=[inp], outputs=[outp])
    w.addJob(j)
    
    j2 = Job(id="foo2", namespace="foo", name="bar", runtime=1024, MI=100000 ,inputs=[outp], parents=[j])
    w.addJob(j2)
